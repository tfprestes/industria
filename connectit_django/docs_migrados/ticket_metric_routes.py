...
# Garantindo que a função get_documents_with_query seja importada para os filtros
...
def _get_filtered_metrics(args: Dict) -> List[Dict[str, Any]]:
    """Busca, filtra e ordena as métricas com base nos argumentos da URL."""
    filter_sector = args.get('filter_sector', '')
    filter_year = args.get('filter_year', '')
    filter_month = args.get('filter_month', '')

    query_filters = []
    if filter_sector:
        query_filters.append({'field': 'sectorId', 'op': '==', 'value': filter_sector})
    if filter_year:
        query_filters.append({'field': 'referenceYear', 'op': '==', 'value': int(filter_year)})
    if filter_month:
        query_filters.append({'field': 'referenceMonth', 'op': '==', 'value': int(filter_month)})
    
    # Usando a função otimizada. A ordenação por múltiplos campos exigiria um índice composto.
    # Por isso, ordenamos no backend para maior flexibilidade.
    metrics = get_documents_with_query('ticketMetrics', query_params=query_filters)
    
    # Ordena os resultados no Python: do mais recente para o mais antigo.
    metrics.sort(key=lambda m: (m.get('referenceYear', 0), m.get('referenceMonth', 0)), reverse=True)
...
def list_metrics() -> str:
    """Exibe a lista de métricas de chamados com filtros e ordenação."""
    try:
        # Primeiro, carregamos os dados para os filtros. Assim, mesmo que a busca falhe, os filtros aparecem.
        context_data = _get_form_context()
        
        # Agora, buscamos os dados da tabela
        all_metrics = _get_filtered_metrics(request.args)
        
        sectors_map = {s['id']: s.get('name', 'N/A') for s in context_data['sectors']}
        months_map = {m['value']: m['name'] for m in context_data['months']}
        ...
        # Prepara o contexto completo para o template
        context = {
            'metrics': all_metrics,
            'all_sectors': context_data['sectors'],
            'all_years': context_data['years'],
            'all_months': context_data['months'],
            'current_filters': {
                'sector': request.args.get('filter_sector', ''),
                'year': request.args.get('filter_year', ''),
                'month': request.args.get('filter_month', '')
            }
        }
        return render_template('ticket_metrics/list.html', **context)
    except Exception as e:
        current_app.logger.error(f"Erro ao listar métricas: {e}", exc_info=True)
        flash("Ocorreu um erro ao carregar as métricas.", "danger")
        # Mesmo em caso de erro, tenta carregar o contexto dos filtros para não quebrar a página
        empty_context = _get_form_context()
        return render_template('ticket_metrics/list.html', metrics=[], 
                               all_sectors=empty_context['sectors'], all_years=empty_context['years'],
                               all_months=empty_context['months'], current_filters={})
...
def export_metrics_csv() -> Response | WerkzeugResponse:
    """Exporta as métricas (respeitando os filtros) para um ficheiro CSV."""
    try:
        metrics_to_export = _get_filtered_metrics(request.args)
        if not metrics_to_export:
            flash("Não há métricas para exportar com os filtros atuais.", "info")
            return redirect(url_for('ticket_metrics.list_metrics'))

        context_data = _get_form_context()
        sectors_map = {s['id']: s.get('name', '') for s in context_data['sectors']}
        units_map = {u['id']: u.get('name', '') for u in context_data['units']}
        cost_centers_map = {cc['id']: cc.get('name', '') for cc in context_data['cost_centers']}
        ...
        output.seek(0)
        return Response(output.getvalue(), mimetype="text/csv", headers={"Content-Disposition": f"attachment;filename=export_metricas_chamados.csv"})
    except Exception as e:
...
def import_metrics_csv() -> str | WerkzeugResponse:
    """Processa o upload de um CSV para importação em massa de Despesas."""
